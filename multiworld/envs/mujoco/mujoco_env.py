...
class MujocoEnv(gym.Env):
    ...
    def __init__(self, model_path, frame_skip, device_id=-1, automatically_set_spaces=False):
        ...
        self.metadata = {
            'render.modes': ['human', 'rgb_array', 'depth_array', 'both_arrays'],
    ...
    def render(self, mode='human'):
        if mode == 'rgb_array':
            ...
            width, height = 500, 500
            # width, height = 4000, 4000
            data = self._get_viewer().read_pixels(width, height, depth=True)[1]
            return data[::-1, :]
        elif mode == 'both_arrays': 
            width, height = 1000, 1000
            self._get_viewer().render(width=width, height=height, mode='offscreen')
            # window size used for old mujoco-py:
            
            # width, height = 4000, 4000
            data = self._get_viewer().read_pixels(width, height, depth=True)
            rgb = data[0]
            depth = data[1]
