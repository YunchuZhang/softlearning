...
from softlearning.map3D.map3D_trainer import MappingTrainer as bulledtPushTrainer
...
class ExperimentRunner():
        ...
        def save(self, sess, name, step):
                ...
                savepath = path.join(self.algorithm.model.ckpt_base,
                                     self.algorithm.model.ckpt_dir,
                                     name)
        ...
        def map_load(self,sess, name,map3D=None):
                # st()
                if not path.exists(path.join(self.algorithm.model.ckpt_base,
                                             self.algorithm.model.ckpt_cfg_dir,
                                             name)):
                        return 0
                config = Config(name)
                config.load()
                ...
                for i in range(self.step,self.step+1000):
                        ...
                        if i % 10 ==0:
                                self.save(self._session,
                                          self.algorithm.model.load_name,
                                          i)
                                num = 0
