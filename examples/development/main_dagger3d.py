...
from softlearning.algorithms.dagger_3d import DAGGER
...
class ExperimentRunner(tune.Trainable):
    ...
    def _build(self):
        ...
        tmp = variant['sampler_params']['type']
        variant['sampler_params']['type'] = 'Simple3DSampler'
        self.sampler = sampler = get_sampler_from_variant(variant)
        variant['sampler_params']['type'] = tmp
        policy = self.policy = get_policy_from_variant(
            variant, training_environment, Qs=[None])
        initial_exploration_policy = self.initial_exploration_policy = (
            get_policy('UniformPolicy', training_environment))

        self.algorithm = DAGGER(
            training_environment=training_environment,
            evaluation_environment=evaluation_environment,
            policy=policy,
            expert_path='/projects/katefgroup/robert/fix2result_hat1/checkpoint_950/',
        ...
        if self.algorithm.pretrained_map3D:
            self.algorithm._map3D_load(self._session, map3D=self.algorithm.map3D)
    ...
    def _restore(self, checkpoint_dir):
        ...
        policy = self.policy = (
            get_policy_from_variant(self._variant, training_environment, Qs=[None]))
