...
def data_options(name, fn, add_suffix = False, split_format=False, split_cut=False):
    train_fn = fn+'_train' if add_suffix else fn
    test_fn = fn+'_test' if add_suffix else fn
    val_fn = fn+'_val' if add_suffix else fn
    
    return OptionGroup(name, train_file = train_fn, test_file = test_fn, val_file = val_fn, split_format=split_format, split_cut=split_cut)
