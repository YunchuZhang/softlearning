...
import ipdb
from softlearning.preprocessors.utils import get_convnet3d_preprocessor
st = ipdb.set_trace
slim2 = Munch()
...
def action_predictor(goal,state_observation,inputs):
	...
	state_observation = tf.squeeze(state_observation)
	goal = tf.squeeze(goal)[:,3:]
	net = tf.concat([net, goal, state_observation], -1)
	#net = tf.layers.dense(net,196,activation=tf.nn.relu)
	net = tf.layers.dense(net,128,activation=tf.nn.relu)
	net = tf.layers.dense(net,96,activation=tf.nn.relu)
	net = tf.layers.dense(net,108,activation=tf.nn.relu)
	net = tf.layers.dense(net,64,activation=tf.nn.relu)
	net = tf.layers.dense(net,32,activation=tf.nn.relu)
	net = tf.layers.dense(net,24,activation=tf.nn.relu)
	net = tf.layers.dense(net,8,activation=tf.nn.relu)
	net = tf.layers.dense(net,2,name = 'final_result')
	return net
